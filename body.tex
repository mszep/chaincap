...
Recently, additional use of the GW approximatin has become common, in order to
correct for the shifting of quasiparticle levels in the device due to coupling
to, and applied bias on, the leads.

Using electronic structure methods from the field of quantum chemistry, it is
possible to incorporate electron correlations to an arbitrary degree, but
modeling the effect of coupling to leads is problematic.

In the conventional approaches, this coupling is modeled by inserting a complex
self-energy term $\Sigma$ into the Hamiltonian. This term however contains a
dependence on the single-particle energy, making it impractical for use with
