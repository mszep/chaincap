...
\ac{LDA} are treated as \acp{QP} and the electron
scattering equations are solved with an external voltage bias
...
treatment of transport in open systems have become commonly used; more
...
Use of the Kohn-Sham energies in the NEGF formulation implies a single
...
electron correlations on \ac{QP} states arising from the device
...
electrodes to study how the \ac{QP} states evolve. 
...
treated~\cite{ness2012jpa_leadnegf}.

With our approach, excitations from the electrode portions that are
included in the explicit device region can be included provided
the electronic structure treatment of the device regions allows for
...
\subsubsection{Derivation of the \ac{CAP}}
...
\subsubsection{Verification of the \ac{CAP}}
...
threshold, while keeping the computational intensity at a manageable
level. Details of the \ac{MCCI} method may be found in 
...
for the 0.40 nm separated junction in the different approximations,
using the Landauer formula. In the top
...
has a later onset, due to the higher bandgap.
...
\begin{figure}
	...
	\caption{Current versus voltage for the various transmission
		 ...
		 potentials on either side are taken to move in opposite
		 directions as voltage is applied.}
	\label{fig:iv}
\end{figure}
