...
program~\cite{times}. These self-energies are added to the single particle
Hamiltonian on the device region, $\um{H}_D$, to yield
\begin{eqnarray}
\label{eq:capseq}
...
Hamiltonion $\um{H}_D$, and $\um{\omega}$ is the eigenvalue matrix
calculated at $\lambda = 1$. In practice, as the self-energies are
independent of the device region for properly defined electrodes and
extended device region, the \acp{CAP} are generated on the electrodes
...
(no orbital relaxation) and a $\Delta$SCF approximation (inclusion of
...
the EAs and IPs eqs.~\ref{eq:EA} and \ref{eq:IP} correspond to a $\Delta$SCF
approximation. 
...
is compensated for this approximate $\Delta$SCF calculation.
...
underestimated and overestimated, respectively, whereas the $\Delta$SCF
...
the Koopmans and $\Delta$SCF estimates.
...
change the broadening relative to the $\Delta$SCF resonances.
...
$\Delta$SCF approximation again shifts the energies of the HOMO and LUMO
...
that introduced at the $\Delta$SCF level. Introducing electron correlations
again renormalizes the energy gap, with the correlated resonant states
bracketed by the Koopmans and $\Delta$SCF values.

\begin{figure}
	\begin{center}
		\includegraphics[width=0.9\linewidth]{figures/all45Ahomo.eps}
	\end{center}
	\caption{HOMO region quasiparticle peaks of the 0.45 nm separated
		 ...
		 $\Delta$SCF (green), and MCCI with threshold parameters of
...
\begin{figure}
	\begin{center}
		\includegraphics[width=0.9\linewidth]{figures/all45Alumo.eps}
	\end{center}
...
\begin{figure}
	\begin{center}
		\includegraphics[width=0.9\linewidth]{figures/all40Ahomo.eps}
	\end{center}
...
\begin{figure}
	\begin{center}
		\includegraphics[width=0.9\linewidth]{figures/all40Alumo.eps}
	\end{center}
...
previously was the case. However, in this case the $\Delta$SCF results
in much larger energy shifts with respect to a $\Delta$SCF approximation
...
are no longer bracketed by the HF + self-energy and $\Delta$SCF peaks.
...
peak from a lifetime of 121 fs from the $\Delta$SCF approximation to 21
fs. 
...
with the $\Delta$SCF approximation as for the 0.45 nm coupling case,
...
LUMO. Hence in $\Delta$SCF approximation to the LUMO, the electrodes and
...
of the LUMO state relative to the $\Delta$SCF approximation changes: the
...
{\bf INTRODUCE CURRENT PREDICTIONS FROM TRANSMISSION HERE?}
...
from the $\Delta$SCF and CI calculations, and for the range of electrode
...
overestimated the QP energy gap, the $\Delta$SCF approximation underestimated
...
and $\Delta$SCF estimates. However, these findings are fundamentally
